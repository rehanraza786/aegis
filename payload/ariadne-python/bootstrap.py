...
Renamed from setup.py: with pyproject.toml in this directory, packaging
tooling executes a file named setup.py — and this one installs git hooks and
runs a full index as side effects, which must never happen during a build.
For the same reason everything now lives under main(): importing this module
does nothing."""
import os
import stat
import subprocess
...
def main():
    ...
    hook_block = _hook_block(repos, multi)
