...
| **Seam extractors** | `.ariadne/extensions/*.extract.mjs` / `*.extract.py` | export `extractors = { kafka, dbAccess, httpEndpoints, httpCalls }`, each `(text, ctx) -> rows[]`; rows land in the NATIVE tables, so they flow through message_flow/db_map/http_map and docgen automatically. A bare function runs over the default java/kotlin set; give a hook a **file scope** — `{ fn, files: /\.go$/ }` (node) or `{"fn": fn, "files": r"\.go$"}` (python) — and it runs over every tracked file the regex matches instead, which is how a Go/Rails/TS-only stack becomes a first-class citizen of the graph. Scoped files join incremental extraction (per-file dirty tracking, deletes, cache) automatically; if your extension isn't in `LANG_BY_EXT` yet, map it with `extraExtensions` in `.ariadne/config.json` so the files are indexed at all. Samples: `spring-cloud-stream.extract.*` (default scope), `eventbus.extract.*` (file-scoped TS). This is the intended path for gateway base-paths, gRPC, GraphQL, or any house framework |
