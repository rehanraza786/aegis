...
From the [latest release](https://github.com/YOUR-USERNAME/aegis/releases/latest), or VS Code → Extensions → `⋯` → *Install from VSIX*.

Open your workspace — one repo or a folder of repos, both work. The extension offers to set itself up; accept, and approve the terminal step (it builds the index and installs git hooks). Open Copilot Chat in agent mode. Done.
...
That's the interface. There's a one-page [cheat sheet](docs/CHEATSHEET.md) your team can pin. **Everything below this line is reference** — useful when you want it, unnecessary to get started.
...
Twenty-four, served over MCP by `ariadne`. You won't type these — they're what the skills call.
...
Several repos open as one VS Code workspace are indexed as **one graph**. Paths are repo-prefixed, incremental indexing diffs each repo independently, and the correlation passes work *across* repos — which is the entire point, since your producer and consumer live in different ones.
